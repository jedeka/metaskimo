...
from .distributions import TanhNormal, Normal
...
class TDMPCModel(nn.Module):
    ...
    def imagine_step(self, state, ac):
        # testlog("shapes in imagine", state.shape, ac.shape)
        # if state.shape[0] != ac.shape[0]:
        #     # if different shape, interpolate
        #     testlog(f'\nchange shape {state.shape[0]} <- {ac.shape[0]:}')
        #     testlog(f'{ac.unsqueeze(1).shape}')
        #     ac = F.interpolate(ac.T.unsqueeze(1), size=(state.shape[0]), mode='nearest')#.squeeze(1)
        #     testlog(ac.shape)
        #     ac = torch.clamp(ac.squeeze(1).T, -0.999, 0.999)
        #     testlog(ac)
        # testlog(f'\n{state.size()}, {ac.size()}', above=True, below=True)
...
class Encoder(nn.Module):
    def __init__(self, cfg, ob_space, state_dim):
        ...
        testlog(ob_space.spaces, enc_dim, state_dim, cfg.dense_act)

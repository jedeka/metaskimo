...
from rolf.networks.distributions import TanhNormal, mc_kl, Normal
...
class SkiMoMetaAgent(BaseAgent):
    ...
    def __init__(self, cfg, ob_space, ac_space):
        ...
        # testlog(ob_space, ac_space, cfg.skill_dim, above=True, below=True)
        
        self.model = TDMPCModel(cfg, self._ob_space, cfg.skill_dim, self._dtype)
        # testlog('\n', self.model, above=True, below=True)
    ...
    def plan(self, ob, prev_mean=None, is_train=True):
        ...
        # testlog(f'state:\n{state}, shape ', state.shape, below=True)

        # Sample policy trajectories.

        # SELF-NOTE: stack all the policy candidates into one tensor 
        z = state.repeat(cfg.num_policy_traj, 1)
        policy_ac = []

        # testlog(f'  PLAN HEREEEEE\n{self._step}\n{horizon}\n{ob}', above=True, below=True)
        
        # testlog('z shape ', z.shape, below=True)
        
        for t in range(horizon):
            skimo_z = self.actor.act(z)
            # testlog(f'{skimo_z.size()}', below=True)
...
class SkiMoAgent(BaseAgent):
    ...
    def __init__(self, cfg, ob_space, ac_space):
        ...
        if cfg.phase == "rl":
            ...
            # testlog(self.skill_prior, above=True, below=True)
    ...
    def _update_network(self, batch):
        ...
        o = self.preprocess(o, aug=self._aug) # preprocess, normalize and augment ob 
        ...
        with torch.no_grad():
            # testlog(f'BEFORE FLIP {self.skill_prior.model.encoder(o).size()}\n', above=True) 
            sp_feat = flip(self.skill_prior.model.encoder(o))
            hl_feat_target = flip(hl_agent.model_target.encoder(o))
        
        """
        SELF-NOTE:
        # batch is from replay buffer
        #o: dict {'ob': tensor}, o['ob]: torch.Size([128, 11, 4])
        sp_feat: tensor torch.Size([11, 128, 128]) # why the result shape is like this?
        """
        # testlog(f'HERE {len(batch)}, {o["ob"].size()}\n', batch, '\n\m', f"{o}, {o['ob'].size()}\n\n", sp_feat, '\n\n', sp_feat.size(), above=True) 
        # testlog(f"\n{'-'*20}\n", self.skill_prior.model.encoder, f"\n{'-'*20}\n")
        # testlog(f"\n{'-'*20}\n", self.skill_prior.actor, f"\n{'-'*20}\n")
        # testlog(f"\n{'-'*20}\n", self.skill_prior.decoder, f"\n{'-'*20}\n")
        # testlog("gradient:", sp_feat.requires_grad, below=True)

        ob = flip(o, cfg.n_skill + 1) # here n_skill + 1, why? -> for t + 1
        ac = flip(ac)
        rew = flip(rew)
        done = flip(done)
        ...
        with torch.autocast(cfg.device, enabled=self._use_amp):
            ...
            for t in range(cfg.n_skill):
                ...
                with torch.no_grad():
                    ...
                    # testlog("PRIOR DIST:\n", t, cfg.n_skill, dir(prior_dist), type(prior_dist), prior_dist.batch_shape, prior_dist.mean, above=True)

                    # testlog(prior_dist.mean, prior_dist.base_dist.std, prior_dist.base_dist.mean)

                    # # testlog(type(prior_dist.mean), type(prior_dist.stddev), prior_dist.mean.size(), prior_dist.stddev.size(), below=True)
        ...
        with torch.autocast(cfg.device, enabled=self._use_amp):
            ...
            for t in range(cfg.n_skill): # why iterate n_skill?
                z = z_next_pred
    ...
    def _pretrain(self, batch, is_train=True):
        ...
        cfg = self._cfg
        B, H, L = cfg.pretrain.batch_size, cfg.skill_horizon, cfg.n_skill # batch, 10, 10
        scalars = cfg.scalars
        hl_agent = self.meta_agent
        ll_agent = self.skill_agent
        info = Info()
        ...
        with torch.autocast(self._cfg.device, enabled=self._use_amp):
            ...
            z_dist = ll_agent.skill_encoder(x) # SELF-NOTE: skill_encoder here (q_theta)
            z = z_dist.rsample() # reparameterization sample  https://stackoverflow.com/questions/60533150/what-is-the-difference-between-sample-and-rsample
            ...
            hl_o = dict(ob=o["ob"][:, ::H])
            hl_feat = flip(hl_agent.model.encoder(hl_o)) # h (latent state)
            with torch.no_grad():
        ...
        #     flip(z_dist.base_dist.loc.detach()),
        #     flip(z_dist.base_dist.scale.detach()),
        #     1,
        # )

        z_dist_detached = Normal( # revert back to tanhnormal later
            flip(z_dist.base_dist.loc.detach()),
            flip(z_dist.base_dist.scale.detach()),
            1,
        )
        ...
        # testlog(meta_ac_dist.base_dist.mean, dir(meta_ac_dist.base_dist), above=True, below=True)
        # info["actor_std"] = meta_ac_dist.base_dist.base_dist.scale.mean().item()
        info["actor_std"] = meta_ac_dist.base_dist.scale.mean().item() # for normal, revert later
